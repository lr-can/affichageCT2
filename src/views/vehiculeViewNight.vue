    ...
    <div class="app-container">
        ...
        <transition name="bubble-fade" mode="out-in">
            <div v-if="currentInstruction" key="currentInstructionIndex" class="instruction-bubble">
                <div class="instruction-titre">{{ currentInstruction.titre }}</div>
                <div class="instruction-meta">
                    <span>{{ formatDuration(currentInstruction.debut) }} → {{ formatTarget(currentInstruction.fin) }}</span>
                    <span class="meta-separator">–</span>
                    <span>{{ currentInstruction.origine }} – {{ currentInstruction.nom }}</span>
                </div>
                <div class="instruction-texte">{{ currentInstruction.texte }}</div>
            </div>
        </transition>
    </div>
</template>
...
.title {
    ...
    margin: 0;
    text-shadow: 0 2px 12px rgba(0, 0, 0, 0.5), 0 0 20px rgba(255, 255, 255, 0.1);
}
